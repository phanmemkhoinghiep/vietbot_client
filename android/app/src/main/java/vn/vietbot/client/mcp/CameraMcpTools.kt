...
BAT BUOC GOI KHI:
- Người dùng yêu cầu: "Bắt đầu quay video", "Quay video dài"
...
- Video lưu vào VietBot/Movies""",
        ...
        McpTool(
            ...
            description = """📊 KIỂM TRA TRẠNG THÁI QUAY VIDEO
...
- Người dùng muốn biết có đang quay video không""",
                                                    ...
                                                    cameraScope.launch(Dispatchers.IO) {
                                                        try {
                                                            ...
                                                            if (description.isNotEmpty()) {
                                                                // AI analysis mode: send image via HTTP to vision server
                                                                // FIX: Send via HTTP instead of WebSocket to avoid 1007 error
                                                                Log.i(TAG, "Sending image to vision server via HTTP...")
                                                                val aiResponse = uploadToVisionServer(rotatedImageData, description)
                                                                cont.resume(McpToolResult(
                                                                    content = listOf(
                                                                ...
                                                                cont.resume(McpToolResult(
                                                                    content = listOf(McpContent.TextContent("Đã chụp ảnh và lưu vào VietBot")),
                                                                    imageUri = galleryUri?.toString()
                                                                ))
                                                            }
                                                        } catch (e: Exception) {
    ...
    /**
     * Upload image to vision server via HTTP multipart/form-data (synchronous version)
     * Called from non-suspend context (e.g., McpServer)
     */
    ...
    /**
     * Upload image to vision server via HTTP multipart/form-data (suspend version)
     */

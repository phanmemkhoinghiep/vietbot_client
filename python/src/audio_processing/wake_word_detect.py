...
class WakeWordDetector:
    ...
    async def _detection_loop(self):
        ...
        """
        error_count = 0
        MAX_ERRORS = 3  # Reduce max errors for faster recovery
        consecutive_errors = 0
        SLEEP_INTERVAL = 0.02  # 20ms = 50 checks/second (optimized from 5ms)

        while self.is_running_flag:
            try:
                if self.paused:
                    await asyncio.sleep(SLEEP_INTERVAL * 2)  # Longer sleep when paused
                    continue
                ...
                # Optimized sleep interval - still responsive but less CPU intensive
                await asyncio.sleep(SLEEP_INTERVAL)
                error_count = 0  # Reset error count on success
                consecutive_errors = 0

            except asyncio.CancelledError:
                logger.debug("KWS检测循环被取消")
                break
            except Exception as e:
                error_count += 1
                consecutive_errors += 1
                logger.error(f"KWS检测循环错误({consecutive_errors}/{MAX_ERRORS}): {e}")
                ...
                backoff_time = min(2 ** consecutive_errors, 5)  # Max 5 seconds
                await asyncio.sleep(backoff_time)

    async def _process_audio(self):
        """处理音频数据 - 改进的批量处理与backpressure控制"""
        try:
            ...
            while self.keyword_spotter.is_ready(self.stream) and detection_count < max_detections:

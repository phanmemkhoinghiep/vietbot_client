...
def get_search_paths(system: PLATFORM, arch_name: str) -> List[Tuple[Path, str]]:
    """
    获取库文件搜索路径列表（使用统一的资源查找器 + fallback paths）
    """
    ...
    # 5. Fallback: 直接使用项目根目录 + libs
    libs_fallback = project_root / "libs"
    if libs_fallback.exists():
        search_paths.append((libs_fallback, lib_name))
        logger.debug(f"添加fallback libs目录: {libs_fallback}")

    # 6. Fallback: 尝试查找系统级opus库
    system_lib_paths = [
        Path("/usr/local/lib"),
        Path("/usr/lib"),
    ...
    for sys_path in system_lib_paths:
        if sys_path.exists():
            search_paths.append((sys_path, lib_name))

...
class MqttProtocol(Protocol):
    # Connection timeout constants (configurable via config)
    DEFAULT_CONNECT_TIMEOUT = 10.0
    DEFAULT_HELLO_TIMEOUT = 10.0
    DEFAULT_MAX_RECONNECT_ATTEMPTS = 3
    DEFAULT_BASE_RETRY_DELAY = 1.0  # Base delay for exponential backoff (seconds)
    DEFAULT_MAX_RETRY_DELAY = 30.0  # Maximum retry delay (seconds)
    ...
    def __init__(self, loop):
        ...
        self._connect_timeout = self.config.get_config(
            "APP.MQTT_CONNECT_TIMEOUT", self.DEFAULT_CONNECT_TIMEOUT
        )
        self._hello_timeout = self.config.get_config(
            "APP.MQTT_HELLO_TIMEOUT", self.DEFAULT_HELLO_TIMEOUT
        )
    ...
    def _parse_endpoint(self, endpoint: str) -> tuple[str, int]:
        ...
        Args:
            endpoint: endpoint string, formats:
                     - "hostname" (use default port 1883)
                     - "hostname:port" (use specified port)
                     - "mqtt://hostname" (use default port 1883)
                     - "mqtts://hostname" (use default port 1883, no TLS)
                     - "mqtt://hostname:port"
                     - "mqtts://hostname:port"
        ...
        # Strip protocol prefix (mqtt:// or mqtts://)
        if endpoint.startswith("mqtts://"):
            endpoint = endpoint[8:]  # Remove "mqtts://"
        elif endpoint.startswith("mqtt://"):
            endpoint = endpoint[7:]  # Remove "mqtt://"
        ...
        if ":" in endpoint:
            ...
            # No port specified, use default port 1883 (plain MQTT)
            host = endpoint
            port = 1883
    ...
    async def connect(self):
        ...
        try:
            ...
            # 等待连接完成 (使用可配置的超时)
            await asyncio.wait_for(connect_future, timeout=self._connect_timeout)
            ...
            try:
                await asyncio.wait_for(self.server_hello_event.wait(), timeout=self._hello_timeout)
            except asyncio.TimeoutError:
                logger.error(f"等待服务器hello消息超时 ({self._hello_timeout}s)")
                if self._on_network_error:
    ...
    def _stop_udp_receiver(self):
        ...
        if (
            ...
            # 尝试优雅关闭线程
            try:
                self.udp_thread.join(timeout=2.0)  # 增加超时到2秒
                if self.udp_thread.is_alive():
                    logger.warning("UDP接收线程未能及时退出，可能仍在运行")
            except RuntimeError:
                pass  # 处理线程已经终止的情况
    ...
    async def _connection_monitor(self):
        """
        连接健康状态监控 - 改进版本，增加检查频率.
        """
        monitor_interval = 15  # 减少到15秒检查一次
    ...
    async def _attempt_reconnect(self, original_reason: str):
        ...
        # 计算退避时间 - 指数退避但有上限
        base_delay = self.DEFAULT_BASE_RETRY_DELAY
        max_delay = self.DEFAULT_MAX_RETRY_DELAY
        retry_delay = min(base_delay * (2 ** (self._reconnect_attempts - 1)), max_delay)

        logger.debug(f"重连延迟: {retry_delay:.1f}秒")

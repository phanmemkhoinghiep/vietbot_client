...
from src.protocols.binary_protocol import BinaryProtocolHandler, BinaryProtocolVersion, MessageType
...
class WebsocketProtocol(Protocol):
    def __init__(self):
        ...
        # Binary protocol handler for encoding/decoding audio packets
        self._binary_protocol = BinaryProtocolHandler(BinaryProtocolVersion.V3)
        self._protocol_version = 3  # Default to version 3
            ...
            async for message in self.websocket:
                ...
                try:
                    if isinstance(message, str):
                        ...
                        # 二进制消息，使用binary protocol解码
                        msg_type, payload = self._binary_protocol.decode_packet(message)

                        if msg_type is None:
                            # Protocol V1 - direct Opus frame
                            if self._on_incoming_audio:
                                self._on_incoming_audio(message)
                        elif msg_type == MessageType.OPUS_AUDIO:
                            # Opus audio data
                            if self._on_incoming_audio and payload:
                                self._on_incoming_audio(payload)
                        elif msg_type == MessageType.JSON_DATA:
                            # JSON data embedded in binary frame
                            try:
                                json_data = json.loads(payload.decode('utf-8'))
                                if self._on_incoming_json:
                                    self._on_incoming_json(json_data)
                            except (json.JSONDecodeError, UnicodeDecodeError) as e:
                                logger.error(f"Failed to decode embedded JSON: {e}")
                        else:
    ...
    async def send_audio(self, data: bytes):
        """
        发送音频数据 (使用binary protocol编码).
        """
        if not self.is_audio_channel_opened():
            return

        try:
            # Encode audio data with binary protocol header
            encoded_data = self._binary_protocol.encode_audio_packet(data)

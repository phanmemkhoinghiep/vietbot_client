...
class McpServer:
    ...
    def add_common_tools(self):
        ...
        # 添加摄像头工具 (skip if cv2 not available)
        try:
            ...
            # Camera tools not available (e.g., cv2 not installed on Pi)
            from src.utils.logging_config import get_logger
            logger = get_logger(__name__)
            logger.warning(f"Camera tools not available: {e}. Skipping camera functionality.")

        # 添加桌面截图工具 (skip if cv2 not available)
        try:
            ...
            # Screenshot tools not available (e.g., cv2 not installed on Pi)
            from src.utils.logging_config import get_logger
            logger = get_logger(__name__)
            logger.warning(f"Screenshot tools not available: {e}. Skipping screenshot functionality.")

        # 添加八字命理工具 (skip if pendulum not available)
        try:
            import importlib
            bazi_module = importlib.import_module("src.mcp.tools.bazi")
            get_bazi_manager = bazi_module.get_bazi_manager

            bazi_manager = get_bazi_manager()
            bazi_manager.init_tools(self.add_tool, PropertyList, Property, PropertyType)
        except (ImportError, AttributeError) as e:
            # Bazi tools not available (e.g., pendulum not installed)
            from src.utils.logging_config import get_logger
            logger = get_logger(__name__)
            logger.warning(f"Bazi tools not available: {e}. Skipping bazi functionality.")

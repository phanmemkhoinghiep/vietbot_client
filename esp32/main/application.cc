...
void Application::Start() {
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        ...
        // 🔥 NEW (2026-06-26): Apply listening_mode từ server hello response
        // - Server gửi "realtime" → SetListeningMode(kListeningModeRealtime) cho mode 3 translation
        // - Server gửi "auto_stop" → SetListeningMode(kListeningModeAutoStop) cho mode 1/2 (giống default)
        // - Server không gửi field → protocol listening_mode() = default AutoStop (backward compatible)
        ListeningMode server_listening_mode = protocol_->listening_mode();
        if (server_listening_mode != listening_mode_) {
            ESP_LOGI(TAG, "🎧 [LISTENING_MODE] Applying server-configured listening mode: %d (was: %d)",
                (int)server_listening_mode, (int)listening_mode_);
            SetListeningMode(server_listening_mode);
        } else {
            ESP_LOGI(TAG, "🎧 [LISTENING_MODE] Server mode matches current: %d (no change)", (int)listening_mode_);
        }
    });
